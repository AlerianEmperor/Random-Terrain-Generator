...
struct Utility
{
	...
	*/
	...
	{
		int len = 0;
		int char_widthritten = 0;
		char* log;
		glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &len);
		if (len > 0)
		{
			log = (char*)malloc(len);
			glGetShaderInfoLog(shader, len, &char_widthritten, log);
	...
	void PrintProgramLog(GLuint program)
	{
		...
		{
			log = (char*)malloc(len);
			glGetProgramInfoLog(program, len, &char_widthritten, log);
